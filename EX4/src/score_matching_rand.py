...
class ScoreMatchingModel(nn.Module):
    ...
    def sample(self, bsz, device, noise = 1.0, x0 = None):
        ...
        for idx, (scalar_sigma_start, scalar_sigma_end) in enumerate(zip(sigmas[0:-1], sigmas[1:])):
            ...
            # Calculate noise to add in this step
            noise_step = torch.randn_like(x) * scalar_sigma_start
            
            # Predict x_0 from x and scalar_sigma_start
            pred_x_0 = self.nn_module_wrapper(x, sigma_start)
            
            # Add new, independent noise at each step
            x = pred_x_0 + noise_step
            
            # Normalize by the sigma_end
